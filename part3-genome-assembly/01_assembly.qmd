...
Copy your read dataset into to the `data` directory in your work directory. Replace the `AXY` with the dataset number! The format of the datasets is `A01` to `B10`. The `B` datasets are paired-end, they have `_1` or `_2` in their filenames. Use the wildcard symbol `*` to catch them both when copying.
...
```bash
megahit -1 data/dataset_BXY_1.fastq -2 data/dataset_BXY_2.fastq -o megahit-assembly -t 4
```

It could take a while, and will print a lot of information to your terminal. You don't have to read everything but you can try to see if you understand some parts.
...
```bash
flye --nano-raw data/dataset_AXY.fastq --genome-size 5M --out-dir flye-assembly --threads 4
```
